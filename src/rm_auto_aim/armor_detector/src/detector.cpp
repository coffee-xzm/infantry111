...
cv::Mat Detector::preprocessImage(const cv::Mat & rgb_img)
{
  ...
  cv::GaussianBlur(gray_img,gray_img,cv::Size(5,5),2.5);
  cv::medianBlur(gray_img,gray_img, 3);
  //cv::Mat kernel =getStructuringElement(cv::MORPH_RECT,cv::Size(5,5));
  //cv::morphologyEx(gray_img,gray_img,cv::MORPH_OPEN,kernel);
  cv::Mat binary_img;
...
bool Detector::isLight(const Light & light)
{
  ...
  bool angle_ok = abs(light.tilt_angle) < l.max_angle || abs(light.tilt_angle) > 180-l.max_angle;

  bool is_light = ratio_ok && angle_ok && light.length > l.min_length;
